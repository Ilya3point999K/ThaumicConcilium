...
public class GolemValetGUI extends GuiContainer {
    ...
    public GolemValetGUI(EntityPlayer player, EntityGolemBase e) {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (this.threat >= 0) {
            ...
            this.fontRendererObj.drawSplitString(StatCollector.translateToLocal("tc.valet.gui.text"), 80, 22, 110, 14540253);
        }
        ...
        if (this.golem.getCore() > -1 && ItemGolemCore.hasInventory(this.golem.getCore())) {
            for (shiftx = 0; shiftx < Math.min(6, slots); ++shiftx) {
                ...
                if (this.golem.getUpgradeAmount(4) > 0) {
                    ...
                    this.drawTexturedModalRect(baseX + 96 + shiftx / 2 * 28, baseY + 4 + shiftx % 2 * 31, 72, 168, 24, 12);
                    shifty = this.golem.getColors(shiftx + ((ContainerGolem) this.inventorySlots).currentScroll * 6);
                    if (shifty > -1) {
                        Color c = new Color(Utils.colors[shifty]);
                        float r = (float) c.getRed() / 255.0F;
                        float g = (float) c.getGreen() / 255.0F;
                        float b = (float) c.getBlue() / 255.0F;
                        GL11.glColor4f(r, g, b, 1.0F);
                        this.drawTexturedModalRect(baseX + 105 + shiftx / 2 * 28, baseY + 7 + shiftx % 2 * 31, 0, 176, 6, 6);
                        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
                    }
                }
            }
        ...
        if (ItemGolemCore.hasInventory(this.golem.getCore())) {
            for(shiftx = 0; shiftx < Math.min(6, slots); ++shiftx) {
                if (this.golem.getUpgradeAmount(4) > 0) {
                    ...
                    var7 = par1 - (baseX + 96 + shiftx / 2 * 28);
                    var8 = par2 - (baseY + 4 + shiftx % 2 * 31);
                    if (var7 >= 0 && var8 >= 0 && var7 < 8 && var8 < 12) {
                        this.mc.playerController.sendEnchantPacket(this.inventorySlots.windowId, shiftx + ((ContainerGolem) this.inventorySlots).currentScroll * 6);
                        return;
                    }
                    var7 = par1 - (baseX + 96 + 16 + shiftx / 2 * 28);
                    var8 = par2 - (baseY + 4 + shiftx % 2 * 31);
                    if (var7 >= 0 && var8 >= 0 && var7 < 8 && var8 < 12) {
                        this.mc.playerController.sendEnchantPacket(this.inventorySlots.windowId, shiftx + slots + ((ContainerGolem) this.inventorySlots).currentScroll * 6);
                        return;
                    }
                }
            }
