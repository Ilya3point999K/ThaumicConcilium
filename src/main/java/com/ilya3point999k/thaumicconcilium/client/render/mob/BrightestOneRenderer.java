...
public class BrightestOneRenderer extends Render {
    int size1 = 0;
    int size2 = 0;

    public BrightestOneRenderer() {
    ...
    public void renderEntityAt(Entity entity, double x, double y, double z, float fq, float pticks) {
        if (!(((EntityLiving)entity).getHealth() <= 0.0F)) {
            ...
            float f10 = 3.5F;
            float f11 = (float)x;
            float f12 = (float)y + 1.5F;
            float f13 = (float)z;
            Tessellator tessellator = Tessellator.instance;
            Color color = new Color(0);
            if (Aspect.getAspect(((BrightestOne)entity).getType()) != null) {
                color = new Color(Aspect.getAspect(((BrightestOne)entity).getType()).getColor());
            }
    ...
    public void doRender(Entity entity, double d, double d1, double d2, float f, float f1) {
        ...
        BossStatus.setBossStatus((BrightestOne)entity, true);
