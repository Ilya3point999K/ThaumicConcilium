...
public class RiftGemRenderer implements IItemRenderer {
    IModelCustom modelBase;
    IModelCustom modelTop;
    private static final ResourceLocation textureRL = new ResourceLocation(ThaumicConcilium.MODID + ":textures/models/gem_base.png");
    private static final ResourceLocation modelBaseRL = new ResourceLocation(ThaumicConcilium.MODID + ":models/gem_base.obj");
    private static final ResourceLocation modelTopRL = new ResourceLocation(ThaumicConcilium.MODID + ":models/gem_top.obj");


    public RiftGemRenderer() {
        this.modelBase = AdvancedModelLoader.loadModel(modelBaseRL);
        this.modelTop = AdvancedModelLoader.loadModel(modelTopRL);
    }
    ...
    public void renderItem(IItemRenderer.ItemRenderType type, ItemStack item, Object... data) {
        ...
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON && player_id == rve_id && mc.gameSettings.thirdPersonView == 0) {
            ...
            if (tag != null) {
                if (tag.getInteger("MAX_GEMS") > 0) {
                    if (tag.hasKey("CURR")) {
                        int curr = tag.getInteger("CURR");
                        ItemStack terra = new ItemStack(TCItemRegistry.terraCastGem);
                        EntityItem terraEntity = new EntityItem(playermp.getEntityWorld(), 0.0, 0.0, 0.0, terra);
                        terraEntity.hoverStart = 0.0F;
                        GL11.glPushMatrix();
                        GL11.glTranslatef(0.0F, 0.33F, 0.0F);
                        GL11.glScalef(0.43F, 0.43F, 0.43F);
                        RenderManager.instance.renderEntityWithPosYaw(terraEntity, 0.0, 0.0, 0.0, 0.0F, 0.0F);
                        ...
                        ItemStack stack = new ItemStack(GameData.getItemRegistry().getObject(tag.getString("NAME" + curr)), 1, tag.getInteger("META" + curr));
